...
- **Import from a neutral home — don't reach into another feature.** Finding the primitive isn't enough; *where you import it from* is part of the decision. If it lives inside another feature's private space — its route folder, its module, a `Feature*` name on generic code — importing it there couples the two: that feature's next refactor breaks yours, and the name now lies about what the code does. Hoist the generic part to a domain-neutral shared module, name it for what it does rather than where it was born, and have both features import that. A second caller is exactly what earns the extraction, so this is not the speculative abstraction §2 warns about — but keep it an extraction: move the code, don't redesign it on the way out.
