...
**A test that has never failed is not a regression test.** Writing the test *after* the fix is the normal order when you're fixing something found in production — but a green test proves only that your implementation does what it does, not that it catches the bug. Prove it: revert the fix, watch the test fail with the signature you actually expect, then restore. A test that stays green against the reverted code is asserting nothing, and it will still be green when someone reintroduces the bug six months from now. Anchor the assertion on the real failure too — the inputs and the wrong output that actually happened — so the next reader learns *why* the guard exists and doesn't cheerfully delete it.
...
**The DRY test applies to rules, not just skills.** An instruction duplicated across a prompt, a config file and a skill is three copies that will drift, and an agent reading all three can't tell which is current — so it follows whichever it saw last, or the one with the most vivid example. When you find a rule being violated, your first move is *not* to write a clearer copy. Count the copies first, and read them against each other. Contradicting sources are the bug; one more source is not the fix.

**A rule that isn't enforced isn't a rule.** If an instruction is already written down, already read in the very session that broke it, and still not followed, more prose will not change that. Either something else the agent also reads contradicts it, or it needs a mechanical guard instead of a sentence — a validation step, a bounded output, a check the work must pass before it counts as done. Prose scales to taste and judgment; it does not scale to invariants. Repeatedly restating a rule that keeps being broken is the writing equivalent of retrying a failing call and expecting a different result.

**Examples outrank rules — audit them against each other.** What you *demonstrate* is what gets copied, whatever the surrounding text forbids. A document that bans something in a rule near the bottom and then shows it dozens of times in the examples above has taught the opposite of what it says, and the examples win because they're concrete. Put the hard rules before the examples, and check every example against the rule it's meant to illustrate. An example that violates its own rule is worse than no example at all.
...
- **A working copy is a cache, not the source.** Before reasoning about the state of a repo, a branch, a deployed config, or anything else you hold a local mirror of, refresh it — `git fetch` first, then read. Local clones go stale silently, and platform tooling that commits on your behalf makes them stale faster than you'd guess. "I checked the repo" is false if you checked a clone you last pulled days ago: you checked a snapshot, and reporting its contents as current is how a careful, well-grounded answer turns out to describe a file nobody has anymore. Same for anything you cached earlier in the session — including a conclusion you drew from it.
